...
- [HMP3P.X](https://github.com/tantanGH/hmp3p) 0.7.0以降
- [HFLAP.X](https://github.com/tantanGH/hflap) 0.2.2以降

以上が動作していることが前提条件となります。メインメモリはお使いの環境にもよりますが、最低でも4MB程度は必要かと思います。

MP3SEL.X自体は68030/68060専用ではありませんが、HMP3P.X/HFLAP.Xには68030あるいは68060専用バイナリがあるため注意してください。
...
いくつかのコマンドラインオプションが指定可能です。詳細は `-h` で表示されるヘルプを参照してください。

例えば `-a` をつけて起動すると `HFLAP.X` 0.9.0以降のスペアナ表示に対応できます。(多少制限はあります)
...
* 0.6.3 (2026/05/31) ... ハイメモリドライバ存在チェック修正
* 0.6.2 (2026/05/02) ... HFLAP.X のスペアナ表示に対応
...
* 0.3.2 (2026/03/13) ... コマンドラインオプション追加。時計の点滅は常時ONに戻した
* 0.3.1 (2026/03/13) ... バッファサイズを指定可能にした。時計の点滅を無効化できるようにした
* 0.3.0 (2026/03/08) ... MP3ハーフレート再生に対応
