...
汎用のファイラではMP3のタグを扱えないため、ディレクトリ名やファイル名で曲データを管理・検索する必要がありますが、MP3SEL.XはMP3のID3タグに対応しているため、アルバム名や曲名を見ながら曲を選択することができます。また、プレイリストを複数作成したり、リピート再生やシャッフル再生を行うこともできます。
...
[HMP3P.X](https://github.com/tantanGH/hmp3p)が動作していることが前提条件となります。メインメモリはお使いの環境にもよりますが、最低でも4MB程度は必要かと思います。
